<#PSScriptInfo
.VERSION 0.1
.GUID 7d2f4a9e-1b5c-4e8d-9f3a-6c8e2d1b5a7f
.AUTHOR Alisson Sol
.COMPANYNAME None
...
# Find OpenSSL for password hashing (included with Git for Windows)
$PasswordHash = $null
$opensslPaths = @(
	"openssl",
...
if (-not $PasswordHash) {
	...
	Write-Output "Install Git for Windows (includes OpenSSL) or install OpenSSL separately."
	exit 1
}
