#!/usr/bin/env pwsh
# GUID: 42002bd8-6ae5-47a9-9f4c-16f13f71a155
...
if (Test-Path $cargoTarget) {
    ...
    Write-Host "  [OK] Removed Rust target directory" -ForegroundColor Green
} else {
...
if (Get-Command bazel -ErrorAction SilentlyContinue) {
    ...
    Write-Host "  [OK] Bazel clean completed" -ForegroundColor Green
} else {
...
if (Test-Path $packageDir) {
    ...
    Write-Host "  [OK] Removed package directory" -ForegroundColor Green
} else {
...
if (Test-Path $rootCargoLock) {
    ...
    Write-Host "  [OK] Removed root Cargo.lock" -ForegroundColor Green
}
