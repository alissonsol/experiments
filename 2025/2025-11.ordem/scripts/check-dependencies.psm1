...
# GUID: 4271fa49-ef1b-4d2d-b427-c094f0a82329
...
function Test-NpmInstalled {
    ...
    if ($npmPath) {
        if (-not $Quiet) {
            try {
                $version = & npm --version 2>&1 | Select-Object -First 1
                Write-Host "  [OK] npm: v$version" -ForegroundColor Green
            } catch {
...
function Test-AllDependencies {
    <#
    .SYNOPSIS
    Checks all required dependencies for the ordem project.
    ...
    Array of tool names to check. Valid values: 'Rust', 'Cargo', 'MSVC', 'Bazel', 'Node', 'npm'
    ...
    .EXAMPLE
    Test-AllDependencies -RequiredTools @('Rust', 'Cargo', 'Node', 'npm')
    ...
    param(
        [bool]$Quiet = $false,
        [string[]]$RequiredTools = @('Rust', 'Cargo', 'MSVC', 'Bazel', 'Node', 'npm')
    )
    ...
    foreach ($tool in $RequiredTools) {
        $isInstalled = switch ($tool) {
            ...
            'Node'  { Test-NodeInstalled -Quiet $Quiet }
            'npm'   { Test-NpmInstalled -Quiet $Quiet }
            default {
