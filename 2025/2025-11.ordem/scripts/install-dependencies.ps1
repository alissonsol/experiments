...
Write-Host "Installing Dependencies for ordem" -ForegroundColor Cyan
...
# ============================================================================
Write-Host "STEP 4: Node.js Installation" -ForegroundColor Cyan
Write-Host "----------------------------------------" -ForegroundColor Cyan
Write-Host ""

if (Test-ToolInstalled -Command "node" -Name "Node.js") {
    $results.NodeJS = $true

    # Also check npm
    if (Get-CommandPath "npm") {
        $npmPath = (Get-Command npm).Path
        $npmVer = npm --version
        Write-Host "[OK] npm found at: $npmPath" -ForegroundColor Green
        Write-Host "  Version: v$npmVer" -ForegroundColor Gray
    }
    ...
    $results.NodeJS = Install-WithWinget -Name "Node.js" -WingetId "OpenJS.NodeJS" -Version $REQUIRED_NODE_VERSION -VerifyCommand "node" -Silent $false

    if (-not $results.NodeJS) {
        Write-Host "WARNING: Node.js installation failed." -ForegroundColor Yellow
        Write-Host "You may need to install it manually from: https://nodejs.org" -ForegroundColor Yellow
        Write-Host ""
    }
}
...
if ($allSuccess) {
    ...
    Write-Host "  1. Navigate to UI directory and run: npm install" -ForegroundColor White
    Write-Host "  2. Navigate to services directory and run: cargo build" -ForegroundColor White
    Write-Host ""
} else {
