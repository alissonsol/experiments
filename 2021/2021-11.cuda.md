...
Those numbers will change each time you execute the application. Yet, they indicate that the total execution time is ~950ms (almost a second). There are other versions of the application in the folder as per the article. One to add blocks (`add_block.cu`) and another for the final grid-based (`add_grid.cu`).
