...
Times are in seconds. I did at least 3 measurements for each combination of version and input size, and averaged the times. I also couldn't run the `add_cuda` for `10^9` due to a segmentation fault (likely need to increase the memory size available for Docker). Clerly, using the GPU pays off as the problem size grows. Profile the versions and check where time is being used!
